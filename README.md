...
## Importar Parquet a Iceberg (si ya tienes sólo archivos Parquet)

Si en lugar de re-ejecutar los jobs batch preferís copiar los ficheros Parquet al MinIO, hay dos escenarios:

- Si copiaste la estructura completa de Iceberg (metadatos + manifests + data files) en `s3a://warehouse/`, Spark/Iceberg reconocerán las tablas sin pasos adicionales.
- Si solo tenés los `.parquet` sin los metadatos de Iceberg, podés registrar los datos en una tabla Iceberg con este helper (ejecutar dentro del contenedor `spark-master`):

```bash
# dentro del repo (una sola vez)
docker exec -it spark-master bash -lc "spark-submit /home/jovyan/jobs/import_parquet_to_iceberg.py s3a://bucket/path/to/gold_parquet/ local.gold.recommendations"
```

El script creará `local.gold.recommendations` si no existe o hará `append` si la tabla ya está registrada.

Alternativa rápida: modificar `queries/gold_queries.py` o `dashboard/streamlit_app.py` para leer directamente los Parquet con `spark.read.parquet(...)` en lugar de `spark.table(...)`.
